...
function makeSpans(first, second = null) {
  const spans = second != null ? `${first}\n${second}` : first;

  // const spans = second != null
  //   ? (<View><Text>{first}</Text> <Text>{second}</Text></View>)
  ...
  render() {
    return (
      <View style={styles.textBoxesContainer}>
        <Text style={styles.digitsStyle}>{this.state.digits && this.state.digits}</Text>
        <Text style={styles.wordsStyle}>{this.state.words && this.state.words}</Text>
      </View>
    );
  }
}
